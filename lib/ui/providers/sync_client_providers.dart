...
/// 2. Call SyncService.performSync()
/// 3. Process the response via SyncCoordinator.processSyncResponse()
/// 4. Return ProcessSyncResult
final connectAndSyncProvider =
    FutureProvider.family<ProcessSyncResult, ConnectParams>((ref, params) async {
  ...
  try {
    // 3. Build and send sync request (get local delta, build request, perform sync)
    // Note: The local delta and request building happens inside SyncService.performSync
    // which uses the server-side coordinator. For client-side, we need a minimal request.
